...
class pid_velocity_fixed_height_controller():
    def __init__(self):
        ...
        self.filtered_alt_deriv = 0.0          # for derivative filter
        self.last_time = 0.0
    ...
    def pid(self, dt, desired_vx, desired_vy, desired_yaw_rate, desired_altitude,
        ...
        # Altitude PID control with filtered derivative
        alt_error = desired_altitude - actual_altitude
        raw_alt_deriv = (alt_error - self.past_alt_error) / dt
        # low-pass filter on derivative (dirty derivative)
        alpha_d = 0.7
        self.filtered_alt_deriv = alpha_d * self.filtered_alt_deriv + (1 - alpha_d) * raw_alt_deriv
        # integral term
        self.altitude_integrator += alt_error * dt
        # PID output
        alt_command = (
            gains["kp_z"] * alt_error
            + gains["kd_z"] * self.filtered_alt_deriv
